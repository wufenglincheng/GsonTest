...
public final class MyReflectiveTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, final TypeToken<T> type) {
        ...
        return new AdapterA<T>(constructor, getBoundFields(gson, type, raw));
    }
    ...
    public static final class AdapterA<T> extends TypeAdapter<T> {
        private final ObjectConstructor<T> constructor;
        private final Map<String, BoundField> boundFields;

        AdapterA(ObjectConstructor<T> constructor, Map<String, BoundField> boundFields) {
        ...
        public T read(JsonReader in) throws IOException {
            ...
            //判断是否是object(其实就是判断开始字符是不是'{'),如果不是就捕获异常
            try {
                in.beginObject();
            } catch (Exception e) {
                //完成位移，为了继续下面的解析。
